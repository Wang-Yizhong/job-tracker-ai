...
const from = process.env.RESEND_FROM || "Acme <onboarding@resend.dev>"; // 强烈建议使用已验证域名的发件人
...
<p><a href="${verifyUrl}" style="display:inline-block;padding:10px 14px;border-radius:8px;background:#4F46E5;color:#fff;text-decoration:none">E-Mail bestätigen</a></p>
<p>Der Link ist 24 Stunden gültig.</p>
<p style="color:#6b7280;font-size:12px">Falls der Button nicht funktioniert, kopiere diesen Link in deinen Browser:<br/>${verifyUrl}</p>
</div>
`,
...
if (error) {
  // 关键：打印完整错误，通常有 name / message / statusCode / details
  console.error("[email] Resend error:", JSON.stringify(error, null, 2));
  // 在开发环境，直接把详细错误冒泡回前端（生产环境可隐藏）
